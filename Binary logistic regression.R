...
# 从Excel文件中读取数据
data <- read_excel("E:/样品-代谢数据处理/代谢-暴露分析/高脂血症-重新分组/差异代谢物-二元逻辑回归-高血脂分组.xlsx")
...
# 将结果写入Excel文件
write_xlsx(results, "E:/样品-代谢数据处理/代谢-暴露分析/高脂血症-重新分组/差异代谢物-二元逻辑回归-高血脂分组-result.xlsx")
...
if (length(significant_results) > 0) {
  ...
  write_xlsx(significant_df, "E:/样品-代谢数据处理/代谢-暴露分析/高脂血症-重新分组/差异代谢物-二元逻辑回归-高血脂分组-filter result-小于1.xlsx")
}
warnings()
